...
class ArtistsList extends StatelessWidget {
  ...
  ArtistsList(this.audioQueryTracks,{Key? key}) : super(key: key);
    ...
    // AudioPlayer player;
     return Center(
       child:StreamBuilder<List<ArtistModel>>(
         // Default values:
         stream: audioQueryTracks.queryArtists(
           sortType: null,
           ...
           return ListView.separated(
             ...
             itemBuilder: (context, index) {
               return InkWell(
                 ...
                 child: ListTile(
                   title: Text(item.data![index].artist,overflow: TextOverflow.ellipsis,),
                   subtitle: Text(item.data![index].numberOfTracks.toString() ?? "-"),
                   leading: QueryArtworkWidget(
                     controller: audioQueryTracks,
                     id: item.data![index].id,
                     type: ArtworkType.AUDIO,
                     nullArtworkWidget: Image.asset(AppImages.emoji2Image),
                   ),
                 ),
               );
             },
